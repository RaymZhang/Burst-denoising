...
def trainburstserveur(model, paths, loss_fn, optimizer, scheduler, name, Nb_frames = 4,batch_size=10, num_epochs = 100, nb_subepoch=1000,save_every=1):
	...
	for epoch in range(num_epochs):
		...
		for path in paths:
			...
			trainloader = torch.utils.data.DataLoader(trainset, shuffle=False, batch_size=batch_size,drop_last=True)
			...
			for subepoch in range(nb_subepoch):
				...
				print('subepoch = %d, loss = %.4f,loss_sfd = %.4f,loss_mfd = %.4f' % (subepoch + 1, loss.data,loss_sfd.data,loss_mfd.data )) #loss_mfd.data
...
Affiche :
	...
	- Les images normales du burst successivement
	- Les images bruitées du burst successivement
	- Les images débruitées du burst successivement
	- Les PSNR :
...
def Show_burst2(Denoiser, SFD, loader, pause, framerate =0.1, tour = 2, check=False):
	Denoiser.eval()
	SFD.eval()
	...
	with torch.no_grad():
		...
		print(' Mean PSNR MFD First = %f '%(PSNRmoyFirst/Compteur))
		print(' Mean PSNR MFD Last = %f '%(PSNRmoyLast/Compteur))
		print(' Mean PSNR MSFD First = %f '%(PSNRmoyMSFDFirst/Compteur))
		print(' Mean PSNR MSFD Last = %f '%(PSNRmoyMSFDFirst/Compteur))
		print(' Mean PSNR SFD First = %f '%(PSNRmoySFDFirst/Compteur))
		print(' Mean PSNR SFD Last = %f '%(PSNRmoySFDLast/Compteur))
